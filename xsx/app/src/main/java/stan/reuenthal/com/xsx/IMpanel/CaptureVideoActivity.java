...
import stan.reuenthal.com.xsx.keyboard.EasyAlertDialog;
...
public class CaptureVideoActivity extends TActionBarActivity implements SurfaceHolder.Callback {
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            // 录制过程中红点闪烁效果
            if (invs % 2 == 0) {
                recordingState.setBackgroundResource(R.mipmap.nim_record_start);
            } else {
                recordingState.setBackgroundResource(R.mipmap.nim_record_video);
            }
            if (invs >= VIDEO_TIMES) {
                stopRecorder();
                sendVideo();
    ...
    public void updateRecordUI() {
        if (recording) {
            recordBtn.setBackgroundResource(R.mipmap.nim_video_capture_stop_btn);
        } else {
            recordBtn.setBackgroundResource(R.mipmap.nim_video_capture_start_btn);
        }
    }
